...
internal static class ContextExtensions
{
    ...
    {
        return Diagnostic.Create(descriptor, location, properties, messageArgs.ToArray<object>());
    }
    ...
    public static void ReportDiagnostic(this SymbolAnalysisContext context, DiagnosticDescriptor descriptor, SyntaxReference syntaxReference, params string[] messageArgs)
    {
        ...
        context.ReportDiagnostic(Diagnostic.Create(descriptor, syntaxNode.GetLocation(), ImmutableDictionary<string, string?>.Empty, messageArgs.ToArray<object>()));
    }
    ...
    {
        context.ReportDiagnostic(Diagnostic.Create(descriptor, location, ImmutableDictionary<string, string?>.Empty, messageArgs.ToArray<object>()));
    }
    ...
    public static void ReportDiagnostic(this OperationAnalysisContext context, DiagnosticDescriptor descriptor, ImmutableDictionary<string, string?>? properties, IInvocationOperation operation, DiagnosticReportOptions options, params string[] messageArgs)
    {
        ...
        {
            context.ReportDiagnostic(Diagnostic.Create(descriptor, memberAccessExpression.Name.GetLocation(), properties, messageArgs.ToArray<object>()));
            return;
        }
